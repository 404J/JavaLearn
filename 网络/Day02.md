...
### NAT 网络地址转换（基于三到四层, 修改 IP，来回的数据必须走均衡器，服务器中的默认网关指向负载均衡服务器）
...
- 基于 DR 模型，负载均衡服务器和 server 之间可以不在同一个局域网内，两者之间可以封装一个数据包，其中包涵原有数据包，然后到达 server 后，需要进行拆解数据包，得到真正的数据包。
